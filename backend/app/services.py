...
if settings.GROQ_API_KEY and len(settings.GROQ_API_KEY) > 10:
    try:
        ...
        print(f"[OK] Groq client initialized (key: {settings.GROQ_API_KEY[:8]}...)")
    except Exception as e:
        print(f"[ERROR] Groq client init failed: {e}")
else:
    print(f"[WARN] GROQ_API_KEY missing or too short: '{settings.GROQ_API_KEY[:8] if settings.GROQ_API_KEY else 'EMPTY'}'")
...
if settings.GEMINI_API_KEY and len(settings.GEMINI_API_KEY) > 10:
    try:
        ...
        print(f"[OK] Gemini client initialized (key: {settings.GEMINI_API_KEY[:8]}...)")
    except Exception as e:
        print(f"[ERROR] Gemini client init failed: {e}")
else:
    print(f"[WARN] GEMINI_API_KEY missing or too short: '{settings.GEMINI_API_KEY[:8] if settings.GEMINI_API_KEY else 'EMPTY'}'")

print(f"[INFO] HuggingFace key present: {bool(settings.HUGGINGFACE_API_KEY and len(settings.HUGGINGFACE_API_KEY) > 10)}")
...
def is_fashion_query(query: str) -> bool:
    """
    Keyword-based fashion query classifier.
    Reliable, instant, and requires no API call.
    """
    query_lower = query.strip().lower()

    # Hard reject clearly non-fashion topics
    non_fashion_keywords = [
        "capital of", "prime minister", "president of", "binary search", "sort a list",
        "python code", "javascript", "algorithm", "database query", "sql", "machine learning",
        "world cup", "cricket", "football", "stock market", "bitcoin", "recipe", "cook",
        "tell me a joke", "write a poem", "history of", "geography", "mathematics", "physics"
    ]
    if any(kw in query_lower for kw in non_fashion_keywords):
        return False

    # Accept anything with a fashion keyword
    fashion_keywords = [
        "wear", "outfit", "style", "styling", "fashion", "clothes", "clothing", "dress", "dressed",
        "shirt", "pant", "pants", "trouser", "jeans", "denim", "skirt", "kurti", "kurta", "saree",
        "lehenga", "suit", "blazer", "jacket", "hoodie", "sweater", "top", "blouse", "crop",
        "cargo", "chino", "jogger", "shorts", "palazzo", "legging", "co-ord", "coord",
        "shoe", "shoes", "sneaker", "sandal", "heel", "boot", "loafer", "footwear",
        "bag", "handbag", "watch", "accessory", "accessories", "jewellery", "jewelry",
        "sunglasses", "belt", "wallet", "color", "colour", "match", "matching", "combination",
        "trend", "trendy", "wardrobe", "look", "looks", "aesthetic", "vibe", "occasion",
        "party", "wedding", "office", "college", "casual", "formal", "ethnic", "traditional",
        "summer", "winter", "monsoon", "season", "festive", "fabric", "cotton", "linen",
        "silk", "denim", "rayon", "georgette", "chiffon", "pattern", "print", "solid",
        "floral", "striped", "checked", "slim fit", "oversized", "regular fit",
        "myntra", "amazon", "flipkart", "meesho", "ajio", "zara", "h&m", "levi",
        "budget", "affordable", "cheap", "expensive", "brand", "buy", "shop", "shopping"
    ]
    if any(kw in query_lower for kw in fashion_keywords):
        return True

    # Short queries with no clear signal — allow through (let the AI stylist handle it)
...
def chat_with_groq(message: str, history: List[Dict[str, str]]) -> str:
    ...
    system_prompt = """You are StyleSense AI, a friendly and knowledgeable fashion assistant.

How to respond:
- Answer the user's question directly and concisely first.
- Keep simple answers to 100-200 words.
- Use bullet points only when listing multiple items or combinations.
- Be conversational and easy to read.

For fashion advice (colors, styling, outfits):
- Give practical, general recommendations based on color theory and styling principles.
- Suggest complete outfit combinations when relevant.
- Keep explanations short — one line per suggestion is enough.

For product recommendations:
- ONLY mention specific brands, prices, or stores if the user explicitly asks for shopping suggestions.
- NEVER invent prices, ratings, availability, or store stock.
- If asked for products, say something like: "You can search for this on Amazon, Flipkart, or Myntra" without making up specific listings.

Never add unnecessary filler, technical jargon, or repeat the question back.
If the question is off-topic from fashion, reply with exactly:
...
"""
    ...
    try:
        ...
        print(f"[ERROR] Groq chat API call failed: {type(e).__name__}: {e}")
        raise
...
def generate_dynamic_analysis_with_groq(filename: str, target_gender: str, colors: List[str]) -> Dict[str, Any]:
    ...
    try:
        ...
        print(f"[ERROR] Groq dynamic analysis fallback failed: {type(e).__name__}: {e}")
        return get_mock_image_analysis(filename, target_gender)
            ...
            json_template = """
{
  ...
  "styling_recommendations": {
    ...
    "matching_accessories": ["accessory 1", "accessory 2"]
            ...
            prompt = (
                f"You are a professional fashion analyst. Analyze this clothing image with maximum precision.\n"
                f"Target gender: {gender_hint}.\n\n"
                f"IMPORTANT: Identify the garment using these specific fashion attributes:\n"
                f"- Garment Type: pick the MOST ACCURATE from {json.dumps(garment_types)}\n"
                f"- Pattern: pick from {json.dumps(FASHION_ATTRIBUTES['patterns'])}\n"
                f"- Neckline: pick from {json.dumps(FASHION_ATTRIBUTES['necklines'])}\n"
                f"- Sleeve Type: pick from {json.dumps(FASHION_ATTRIBUTES['sleeve_types'])}\n"
                f"- Silhouette: pick from {json.dumps(FASHION_ATTRIBUTES['silhouettes'])}\n"
                f"- Fit: pick from {json.dumps(FASHION_ATTRIBUTES['fits'])}\n"
                f"- Fabric (estimate): pick from {json.dumps(FASHION_ATTRIBUTES['fabrics'])}\n"
                f"- Occasion: pick from {json.dumps(FASHION_ATTRIBUTES['occasions'])}\n\n"
                "CRITICAL RULES:\n"
                "1. Be PRECISE about the garment type. A Gown is NOT a Kurti. A Jumpsuit is NOT a Dress.\n"
                '2. The "detected_item" must combine key attributes, e.g.: "Women\'s V-Neck Floral Georgette Maxi Gown".\n'
                "3. For \"category\", map correctly:\n"
                ...
                f"Set the \"gender\" field to \"{gender_hint}\".\n"
                "Return ONLY a raw JSON object (no markdown, no explanation) matching this structure:\n"
                + json_template
            )
            ...
            print(f"[ERROR] Gemini image analysis failed: {type(e).__name__}: {e}")
...
def generate_outfit_recommendations(gender: str, occasion: str, budget: float, style_preference: str) -> List[Dict[str, Any]]:
    ...
    try:
        ...
        print(f"[ERROR] Groq outfit generator failed: {type(e).__name__}: {e}")
        return get_mock_outfits(gender, occasion, budget, style_preference)
